...
router.get("/upi/rules", requireAuth, async (_req, res) => {
  try {
    const rules = await db
      .select({
        id: upiRules.id, keyword: upiRules.keyword,
        accountId: upiRules.accountId, accountCode: upiRules.accountCode,
        active: upiRules.active, createdAt: upiRules.createdAt,
...
router.post("/upi/rules", requireAuth, async (req, res) => {
  try {
    ...
    const [rule] = await db.insert(upiRules).values({
      id: randomUUID(), keyword: keyword.trim().toLowerCase(),
      accountId: acct.id, accountCode: acct.code, active: true,
    }).returning();
...
router.get("/upi/accounts", requireAuth, async (_req, res) => {
  try {
    const accts = await db.select({
      id: accounts.id, code: accounts.code, name: accounts.name,
      group: accounts.group, subGroup: accounts.subGroup,
