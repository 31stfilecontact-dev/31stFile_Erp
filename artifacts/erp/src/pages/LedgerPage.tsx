...
export default function LedgerPage() {
  const [, params] = useRoute("/ledger/:code");
  ...
  function downloadCSV() {
    ...
    const rows = data.lines.map(l => [l.entryDate, l.voucherNo, `"${l.narration.replace(/"/g, '""')}"`, l.dr || "", l.cr || "", l.balance, l.balanceDrCr]);
    ...
    <div className="max-w-5xl mx-auto space-y-4">
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <button onClick={() => setLocation("/accounts")} className="btn-ghost" style={{ padding: "8px 10px" }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <button onClick={downloadCSV} disabled={!data || loading} className="btn-ghost" style={{ fontSize: 12, gap: 4 }}>
            <Icon name="download" size={16} /> CSV
          </button>
          <button onClick={() => window.print()} className="btn-ghost" style={{ fontSize: 12, gap: 4 }}>
      ...
      <div className="glass-card p-4">
        <div style={{ display: "flex", gap: 12, alignItems: "flex-end", flexWrap: "wrap" }}>
          <div>
            ...
            <input type="date" className="input-field" style={{ width: 160 }} value={from} onChange={e => setFrom(e.target.value)} />
          </div>
          <div>
            <label className="label-field">To</label>
            <input type="date" className="input-field" style={{ width: 160 }} value={to} onChange={e => setTo(e.target.value)} />
          </div>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            {[
              ...
              { label: "This Month", f: `${new Date().getFullYear()}-${String(new Date().getMonth() + 1).padStart(2, "0")}-01`, t: new Date().toISOString().split("T")[0] },
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(170px,1fr))", gap: 12 }}>
            {[
              ...
              <div key={c.label} className="glass-card p-4" style={{ borderTop: `3px solid ${c.clr}` }}>
                ...
                {c.sub && (
                  <span style={{ fontSize: 10, padding: "1px 6px", borderRadius: 9999, fontWeight: 700, background: `${c.clr}18`, color: c.clr, fontFamily: "'Plus Jakarta Sans',sans-serif" }}>{c.sub}</span>
                )}
              </div>
            ))}
          </div>
